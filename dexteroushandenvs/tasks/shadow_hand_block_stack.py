...
class ShadowHandBlockStack(BaseTask):
    ...
    def compute_observations(self):
        ...
        self.block_right_handle_pos = self.rigid_body_states[:, 26 * 2 + 2, 0:3]
        self.block_right_handle_rot = self.rigid_body_states[:, 26 * 2 + 2, 3:7]
        self.block_right_handle_pos = self.block_right_handle_pos + quat_apply(self.block_right_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * -0.1)
        ...
        self.block_left_handle_pos = self.block_left_handle_pos + quat_apply(self.block_left_handle_rot, to_torch([0, 1, 0], device=self.device).repeat(self.num_envs, 1) * 0.07)
...
def compute_hand_reward(
    ...
    up_rew = (torch.norm(block_right_handle_pos - block_left_handle_pos, p=2, dim=-1) - 1.7) * 20
