...
class ShadowHandDoorCloseInward(BaseTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_asset_options.vhacd_params.resolution = 500000
    ...
    def compute_observations(self):
        ...
        self.door_left_handle_pos = self.door_left_handle_pos + quat_apply(self.door_left_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * -0.4)
        ...
        self.door_right_handle_pos = self.door_right_handle_pos + quat_apply(self.door_right_handle_rot, to_torch([1, 0, 0], device=self.device).repeat(self.num_envs, 1) * 0.4)
...
def compute_hand_reward(
    ...
    right_hand_finger_dist = (torch.norm(door_left_handle_pos - left_hand_ff_pos, p=2, dim=-1) + torch.norm(door_left_handle_pos - left_hand_mf_pos, p=2, dim=-1)
    ...
    left_hand_dist_rew = right_hand_finger_dist
    ...
    up_rew = (door_right_handle_pos[:, 0] + door_left_handle_pos[:, 0]) * 5
    ...
    reward = 1 - right_hand_dist_rew - left_hand_dist_rew
